...
import { Plus, ChevronDown, ChevronRight, Trash2, Upload, Link } from "lucide-react";
...
export default function SubjectManagement() {
  ...
  const startEditContent = async (content: Content) => {
    ...
    setUploadMode(content.url ? 'url' : 'upload');
              ...
              <div className="space-y-2">
                ...
                <Tabs value={uploadMode} onValueChange={(v: any) => setUploadMode(v)}>
                  <TabsList className="grid w-full grid-cols-2">
                    <TabsTrigger value="upload" data-testid="tab-upload-content">
                      <Upload className="w-4 h-4 mr-2" />
                      Upload File
                    </TabsTrigger>
                    <TabsTrigger value="url" data-testid="tab-url-content">
                  ...
                  <TabsContent value="upload" className="space-y-2 mt-4">
                    ...
                    <Input
                      id="contentFile"
                      type="file"
                      accept={contentType === 'video' ? '.mp4,.mov,.avi,.mkv,.webm' : '.pdf'}
                      onChange={(e) => {
                  ...
                  <TabsContent value="url" className="space-y-2 mt-4">
                    <Label>{contentType === 'video' ? 'Video URL' : 'PDF URL'}</Label>
                    <Input 
                      value={contentUrl}
                      onChange={(e) => setContentUrl(e.target.value)}
                      placeholder={contentType === 'video' ? "https://example.com/video.mp4" : "https://example.com/document.pdf"}
                      data-testid="input-content-url"
                    />
                    <p className="text-xs text-muted-foreground">
                      Enter a direct link to a {contentType === 'video' ? 'video' : 'PDF'} file
                    </p>
                  </TabsContent>
                </Tabs>
              </div>
                ...
                onClick={() => {
                  if ((contentType === 'video' || contentType === 'pdf') && !contentUrl.trim()) {
                    toast({ 
                      ...
                      description: `Please ${uploadMode === 'upload' ? 'upload a file or enter' : 'enter'} a ${contentType} URL.`,
                      variant: "destructive" 
                    });
                    return;
                  }
